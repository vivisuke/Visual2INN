...
func _draw():
	...
	style_box.shadow_size = 8 if !selected else 1		# 影（ぼかし）サイズ
	...
	#
	var wd = NODE_RADIUS * 2
	var ht = (Y_X2 - Y_X1) + NODE_RADIUS * 2
	draw_rect(Rect2(Vector2(X_SOFTMAX-wd/2, Y_P1-NODE_RADIUS), Vector2(wd, ht)), Color.WHITE)
	draw_rect(Rect2(Vector2(X_SOFTMAX-wd/2, Y_P1-NODE_RADIUS), Vector2(wd, ht)), Color.BLACK, false, 2.0)
